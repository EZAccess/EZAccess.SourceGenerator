using EZAccess.SourceGenerator.Abstractions.BaseClasses;
using EZAccess.SourceGenerator.DatabaseDefinition;
using System.Linq;

namespace EZAccess.SourceGenerator.Builders;

internal class BFFServiceClassBuilder : CodeBuilderBase
{
	private readonly TableDef _table;
	private readonly string _namespaceName;

	public BFFServiceClassBuilder(TableDef table, string namespaceName)
	{
		_table = table;
		_namespaceName = namespaceName;
	}

	internal static string Build(TableDef table, string namespaceName)
		=> new BFFServiceClassBuilder(table, namespaceName).BuildInternal();

	private string BuildInternal()
	{
		var columns = _table.Fields.Where(f => !f.BackendOnly).ToList();
		var keyFields = _table.Fields.Where(f => f.IsKey).ToList();
		string keyFieldType;
		if (keyFields.Count > 1) { 
			keyFieldType = $"({string.Join(", ", keyFields.Select(f => f.GetDotNetDataType()))})";
		}
		...
		var baseClassName = $"BFFServiceBase<{_table.EntityNameSingular}, {_table.EntityNameSingular}_CoreDTO, {_table.EntityNameSingular}_RequestDTO, {keyFieldType}>";
		...
		BreakLine();
		StartBlock($"public partial interface I{_table.EntityNameSingular}Service2 : IBFFServiceBase<{_table.EntityNameSingular}, {_table.EntityNameSingular}_CoreDTO, {_table.EntityNameSingular}_RequestDTO, {keyFieldType}>");
		{
		}
		EndBlock();

		BreakLine();
		StartBlock($"public partial class {_table.EntityNameSingular}Service2 : {baseClassName}");
		{
			AppendLine($"protected override string RequestUri => \"api/{_table.EntityNameSingular}\";");
			AppendLine($"protected override string EntityName => \"{_table.EntityNameSingular}\";");
			BreakLine();

			StartBlock($"public {_table.EntityNameSingular}Service2(HttpClient httpClient, ILogger<{baseClassName}> logger, IMemoryCache cache) : base(httpClient, logger, cache)");
			EndBlock();
			BreakLine();

			StartBlock($"protected override string GetQueryForModel({_table.EntityNameSingular}_RequestDTO request)");
			{
